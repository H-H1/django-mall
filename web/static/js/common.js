...
function buildUrl(e, t) {
	switch (e) {
	case 'keyword':
		return '/shop/product_list.html?keyword=' + encodeURIComponent(t);
	case 'special':
		return '/special.html?special_id=' + t;
	case 'goods':
		return '/shop/product_detail.html?goods_id=' + t;
	case 'url':
		return t
	}
	return WapSiteUrl
...
$.ajax({
	type: 'get',
	url: RealApiUrl + '/index.php?act=check_weixin&ref=' + encodeURIComponent(window.location.href),
	async: false,
	dataType: 'json',
	success: function(e) {
		if (e.datas == 1) {
...
if (WeiXinOauth) {
	...
	if (ua.match(/MicroMessenger/i) == 'micromessenger') {
		var noauto=getQueryString("noauto");
		if (key == null && (noauto==0 || noauto=='')) {
			window.location.href = RealApiUrl + '/index.php?act=getauthinfo&op=login&ref=' + encodeURIComponent(window.location.href);
		}
	}
	$.ajax({
		type: 'get',
		url: RealApiUrl + '/index.php?act=check_weixin&op=weixin_jssdk&ref=' + encodeURIComponent(window.location.href),
		async: false,
		dataType: 'json',
		success: function(result) {
			if (result.datas) {
				share_config = {
					...
					desc: result.datas.desc,
					img_url: result.datas.img_url,
					link: result.datas.link
				};
			}
		}
	});
...
if (key) {
	$.ajax({
		type: 'get',
		url: ApiUrl + '/index.php?act=distributor&op=deal_distributor&key='+key+'&ref=' + encodeURIComponent(window.location.href),
		async: false,
		dataType: 'json',
